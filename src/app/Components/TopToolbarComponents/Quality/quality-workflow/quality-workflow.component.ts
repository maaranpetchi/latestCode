...
export class QualityWorkflowComponent implements OnInit {
  ...
  calculateElapsedTime(startTime: string): string {
    ...
    return elapsedMinutes.toString();
  }

  changeWorkType1(worktype:string) {
   let fd = new FormData();
    ...
    this.http.post<any>(environment.apiURL+`Workflow/ChangeWorkflow/${this.data.wftid}`, fd).subscribe(data => {
      console.log(data, "start");
      this.BindWorkDetails()
      // this.jobWorkDetails = data
    })
  }
